...
export function AdministrativeUnit() {
     ...
     const getList = async () => {
          setIsloading(true)
          let list = await axios.get(`http://localhost:3001/post/byAdmnsId/${id}`)
          if (list.status === 200) {
     ...
     const getUnitData = async () => {
          setIsloading(true)
          let list = await axios.get(`http://localhost:3001/admns/admnsId/${id}`)
          if (list.status === 200) {
     ...
     const CreatePost = (data) => {
          ...
          setIsloading(true)
          if (isEdit) {
               axios.put("http://localhost:3001/post/updatePost", {
                    ...data, id: editItemId
               },
                    {
     ...
     const deletePost = () => {
          setIsloading(true)
          axios.delete(`http://localhost:3001/post/${deleteItem}`, {
               headers: { accessToken: localStorage.getItem("accessToken") },
          })
               .then(() => {
                    setDeleteItem('');
